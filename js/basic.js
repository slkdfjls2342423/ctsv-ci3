...
function lock_entry(redir_cont){
	
	if(confirm("Bạn có chắc là muốn khóa không?")){
		window.location=base_url+redir_cont;
	}
	
}

function unlock_entry(redir_cont){
	
	if(confirm("Bạn có chắc là muốn mở khóa không?")){
		window.location=base_url+redir_cont;
	}
...
function addquestion(quid,qid){
	...
	$.ajax({
		...
		success: function(data){
			...
			//else
			//	alert(document.getElementById('full_time').value);
		},
		error: function(xhr,status,strErr){
			alert(status);
			}	
...
function movequestion(){
	...
	window.location=base_url+"quiz/down_question/"+global_quid+"/"+global_qid+"/"+npos;
	}
}

function movequestion_position(pos){

	if(position_type=="Up"){
	var npos=parseInt(global_opos)-parseInt(pos);
		window.location=base_url+"quiz/up_question/"+global_quid+"/"+global_qid+"/"+npos;
	}else{
		var npos=parseInt(pos)-parseInt(global_opos);
		window.location=base_url+"quiz/down_question/"+global_quid+"/"+global_qid+"/"+npos;
	}
...
function no_q_available(lid){
	...
	$.ajax({
		 type: "POST",
		 data : formData,
			url: base_url + "quiz/no_q_available/"+cid+'/'+lid,
		success: function(data){
...
function format_category(catid) {
	$("a[attr='link_cat']").removeClass('btn-primary');
	$("a[attr='link_cat']").addClass('btn-info');
	$("#link_cat_"+catid).removeClass('btn-info');
	$("#link_cat_"+catid).addClass('btn-primary');
}

function show_question(vqn){
	//Hien thi gio cho cau hien tai
	if(q_timer[vqn]>0)
	{
		
		document.getElementById("number_of_question").innerHTML = (parseInt(vqn)+1)+" : ";
		document.getElementById("timer_of_question").innerHTML = LeadingZero(q_timer[vqn]);
		$('#box_timer_of_question').css('visibility','visible');
		
		if(q_timer[vqn] == q_durations[vqn])
			q_timer[vqn]--;
	}else if(q_timer[vqn]==0)
		$('#box_timer_of_question').css('visibility','hidden');
	else
	{
		document.getElementById("number_of_question").innerHTML = (parseInt(vqn)+1)+" : ";
		document.getElementById("timer_of_question").innerHTML = time_over_question;// LeadingZero(q_timer[vqn]);
		$('#box_timer_of_question').css('visibility','visible');
	}
		
	var catid=$("#qbtn"+vqn).attr("catid");
	//Change color category
	format_category(catid);
	////////////////////////////
	change_color(vqn);
	fide_all_question();
	var did="#q"+vqn;
...
function end_quiz(){
	...
	window.location=base_url+"index.php/quiz/end_quiz/";
}
...
function switch_category(c_k){
	...
	//$("a[attr='link_cat']").removeClass('btn-primary');
	//$("a[attr='link_cat']").addClass('btn-info');
	// var link=document.getElementById('link_'+c_k);
...
function sort_result(limit,val){
	window.location=base_url+"result/index/"+limit+"/"+val;
...
//Disable a question
function diable_question(vqn) {
	var did="#q"+vqn;
	$(did+' :radio:not(:checked)').attr('disabled', true);
	$(did+" input[type='checkbox']").attr('readonly',true);
	$(':checkbox[readonly]').click(function(){
		return false;
	});
	$(did+" textarea").prop('readonly',true);
}

function check_form_insert_category() {
	$flag=true;
	...
	{
		$flag = false;
		$("#part_name_err").html("Please choose a part");
	}

	return $flag;
}

//Update part of category
function update_pid_category(vall,cid){
	 console.log(base_url + "index.php/qbank/update_pid_category/"+cid);
	var formData = {pid:vall};
	$.ajax({
		 type: "POST",
		 data : formData,
			url: base_url + "index.php/qbank/update_pid_category/"+cid,
		success: function(data){
		$("#message").html(data);
...
function choose_category(id) {
	window.location.href=base_url+"qbank/index/0/"+id;;
}
...
function fnExcelReport() {
	...
	tab_text = tab_text + '<x:Name>Ket qua</x:Name>';
	...
	if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
			if (window.navigator.msSaveBlob) {
					...
					navigator.msSaveBlob(blob, 'result.xls');
			}
	} else {
			
			$('#report').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
			$('#report').attr('download', 'result.xls');
	}
