...
class Quiz extends CI_Controller {
	...
	public function index($limit='0',$list_view='grid')
	{
		...
		$data['notifications']=$this->notification_model->new_notifications($limit);
...
function open_quiz($limit='0'){
		...
		public function insert_quiz()
	{
		...
		//$this->form_validation->set_rules('txtstartdate', 'start_date', 'required');
		//$this->form_validation->set_rules('txtenddate', 'end_date', 'required');
	...
	public function quiz_detail($quid){
		...
		{ 
			...
			//$quiz_time=$this->quiz_model->get_quiz_time($quid);
			$quiz_time=$this->quiz_model->get_quiz_time_group($quid,$gid);
			if(count($quiz_time)==0)
			{
			...
			$data['quiz']['start_date']=$quiz_time['start_date'];
			$data['quiz']['end_date']=$quiz_time['end_date'];
			if($this->survey_model->check()==0)
			{	
				$this->session->set_flashdata('message', "<div class='alert alert-danger'>Bạn hãy vui lòng hoàn thành khảo sát trước khi làm bài<br><a href='".base_url()."survey/add'>Khảo sát</a></div>");
				$data['survey']=0;
			}else
				$data['survey']=1;
		}else
	...
	public function validate_quiz($quid){
		...
		$logged_in=$this->session->userdata('logged_in');
		//$quiz_time=$this->quiz_model->get_quiz_time($quid);
		$quiz_time=$this->quiz_model->get_quiz_time_group($quid,$logged_in['gid']);
		if($data['quiz']['demo']=='0')
		{
			if(count($quiz_time)==0 )
			{
		...
		$start=new DateTime($data['quiz']['start_date']);
		$end=new DateTime($data['quiz']['end_date']);
		...
		if(!in_array($gid,explode(',',$data['quiz']['gids']))){
		 ...
		 //echo "satrt_date: ",$data['quiz']['start_date'];
		// $t= time();
		// echo $data['quiz']['start_date'],"<br>";
		// echo date('Y',$data['quiz']['start_date']),"-";
		// echo date('m',$data['quiz']['start_date']),"-";
		// echo date('d',$data['quiz']['start_date'])," ";
		// echo date('H',$data['quiz']['start_date']),":";
		// echo date('i',$data['quiz']['start_date']),":";
		// echo date('s',$data['quiz']['start_date']),"<br>";
		// echo $t,"<br>";
		// echo date('Y',$t),"-";
		// echo date('m',$t),"-";
		// echo date('d',$t)," ";
...
if(isset($_FILES['webcam'])){
 ...
 function submit_quiz(){
	 ...
	 $rid=$this->session->userdata('rid');
				...
				if($this->quiz_model->submit_result()){
					...
					$user=$this->user_model->get_user($logged_in['uid']);
					if($user['gid']=='3')
					{
						$message = "<div class='alert alert-warning'><a href='https://docs.google.com/forms/d/1u-ROaRKyqUlFo9kFMGw0q8xkDDWk_DVyW-mTQVlGWX8/viewform?edit_requested=true&fbclid=IwAR0yN6CVGGh4G7ryaQ01bU0r62j4ftNImXdDqpipNWz4cO-kiKZrhusq2pw'>Khảo sát ý kiến của sinh viên về thư viện</a></div>";
						$message .= "<div class='alert alert-success'>".str_replace("{result_url}",site_url('result/view_result/'.$rid),$this->lang->line('quiz_submit_successfully'))." </div>";
					}else
						$message = "<div class='alert alert-success'>".str_replace("{result_url}",site_url('result/view_result/'.$rid),$this->lang->line('quiz_submit_successfully'))." </div>";					 
 ...
 function syn_time($rid)
 {
	 $quiz=$this->quiz_model->quiz_result($rid);
	 $seconds=max(0,($quiz['duration']*60) - (time()- $quiz['start_time']));
	 echo $seconds;
 }
 

 //Thiet lap thoi gian thi
 function time()
 {
	...
	$data['time_list']=$this->quiz_model->time_list();
	$data['group_list']=$this->user_model->group_list();
	$data['faculty_list']=$this->faculty_model->faculty_list();
	$data['title']='Thiết lập thời gian thi';

	$this->load->view('header',$data);
	$this->load->view('testing_time',$data);
	$this->load->view('footer',$data);
 }

	 //Thiet lap thoi gian thi cho cac khoa
	 function insert_time()
	 {
		...
		{
			if($this->quiz_model->insert_time()){
	 ...
	 function remove_time($tid)
	 {
		...
		if($this->quiz_model->remove_time($tid)){
      $this->session->set_flashdata('message', "<div class='alert alert-success'>".$this->lang->line('removed_successfully')." </div>");
		}else
		$this->session->set_flashdata('message', "<div class='alert alert-danger'>".$this->lang->line('error_to_remove')." </div>");
		redirect('quiz/time/');
	...
	//Bang: 28-82021
	//An 1 bai thi
	public function hide_quiz($quid)
	{
				// redirect if not loggedin
		if(!$this->session->userdata('logged_in')){
			redirect('login');
		...
		$quid=$this->quiz_model->hide_quiz($quid);
		redirect('quiz');       

	}
	//Bang: 28-82021
	//hien 1 bai thi
	public function show_quiz($quid)
	{
				// redirect if not loggedin
		if(!$this->session->userdata('logged_in')){
			redirect('login');
		...
		$quid=$this->quiz_model->show_quiz($quid);
		redirect('quiz');       

	}
	//Bang: 28-82021
	//hien 1 bai thi
	public function demo_quiz($quid)
	{
				// redirect if not loggedin
		if(!$this->session->userdata('logged_in')){
			redirect('login');
		...
		$quid=$this->quiz_model->demo_quiz($quid);
		redirect('quiz');       
