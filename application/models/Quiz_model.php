...
Class Quiz_model extends CI_Model
{
 ...
 function update_quiz($quid){
	 ...
	 'quiz_name'=>$this->input->post('quiz_name'),
	 'description'=>$this->input->post('description'),
	 //'start_date'=>strtotime($this->input->post('txtstartdate')),
	 //'end_date'=>strtotime($this->input->post('txtenddate')),
	 'start_date'=>$this->input->post('txtstartdate'),
	 'end_date'=>$this->input->post('txtenddate'),
	 'duration'=>$this->input->post('duration'),
	 'maximum_attempts'=>$this->input->post('maximum_attempts'),
...
function get_qcl($quid){
 ...
 function get_quiz_time($quid){
	/*$query_str="SELECT * from savsoft_time WHERE gid in (SELECT gids FROM savsoft_quiz) AND facultyid='".$this->session->userdata('logged_in')['facultyid']."'";
	...
	*/
	...
	$query_str="SELECT * from savsoft_time WHERE gid in (".$gids.") AND facultyid='".$this->session->userdata('logged_in')['facultyid']."'";
	 $query=$this->db->query($query_str);
	//echo $this->db->last_query();exit;
	return $query->row_array();
}
...
if($this->config->item('allow_result_email')){
 ...
 function time_list($limit=0)
	 {
		 $this->db->select('id,savsoft_group.*,savsoft_faculty.*,start_date,end_date');
		if($limit>0)
			$this->db->limit($limit);
		$this->db->order_by('id','desc');
		$this->db->join('savsoft_group','savsoft_time.gid=savsoft_group.gid');
		$this->db->join('savsoft_faculty','savsoft_time.facultyid=savsoft_faculty.facultyid');
		$query=$this->db->get('savsoft_time');

		return $query->result_array();
	 }

	//Them thiet lap ngay gio thi cho cac khoa
	function insert_time()
	{
		...
		if($this->db->insert('savsoft_time',$userdata))
		{
			return true;
			echo $this->db->last_query();exit;
		}else
	...
	function remove_time($tid){
		...
		if($this->db->delete('savsoft_time')){
	...
	//Bang: 28-8-2021
	//An 1 bai thi
	function hide_quiz($quid){
	 
		$userdata=array('demo'=>-1);
		$this->db->where('quid',$quid);
		return $this->db->update('savsoft_quiz',$userdata);
	}
	//Bang: 28-8-2021
	//Hien 1 bai thi
	function show_quiz($quid){
	 
		$userdata=array('demo'=>0);
		$this->db->where('quid',$quid);
		return $this->db->update('savsoft_quiz',$userdata);
	}
	//Bang: 28-8-2021
	//Hien 1 bai thi
	function demo_quiz($quid){
