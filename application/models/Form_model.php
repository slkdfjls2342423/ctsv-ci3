...
class Form_model extends CI_Model
{
	...
	function danhSachSinhVienGiayXacNhan($formid = 10)
	{
		//$query=$this->db->query('select * from savsoft_users');
		$query = $this->db->get_where('v_form_student', array('formid' => $formid));
...
FROM
	...
	function store()
	{
		$_SESSION['store_dc'] = $_POST;
		$operation = $this->input->post('operation');
		$get_at = $this->input->post('get_at') ? $this->input->post('get_at') : 'P.CTSV';
		$ReceivingAddress =  $this->input->post('ReceivingAddress');
		...
		if ($operation == "Edit") {
			...
			$this->db->where('id', $id);
			$this->db->set(['data' => json_encode($_POST), 'date2' => $date2, 'status' => $status, 'note' => $note, 'get_at' => $get_at, 'ReceivingAddress' => $ReceivingAddress]);
			$this->db->update('etp_form_student');
			echo 'Data Updated'; //, id='. $insert_id .'!!!';
			//$_SESSION['store']=$this->db->last_query();
 ...
 */
	...
	function form_nguoiky($idnguoiky)
	{
		$data = $this->db->query("select * from etp_leader where id='$idnguoiky' ")->result_array();
		return $data[0];
	}
...
*/
	...
	//tra ve mang so luong tung loai don trong hoc ky
	function countFormbySemester()
	{
		$user = $this->session->userdata('logged_in');
		$mssv = $user['studentid'];
		$year = Date("Y");
		...
		if ($hocky == 2)
			$sql = "select etp_form.formid, Count(etp_form_student.id) as dem from etp_form   JOIN etp_form_student  on etp_form.formid=etp_form_student.formid
where studentid='$mssv' and date1 >='$year-07-01' and date1<= '$year-12-31'
GROUP BY etp_form.formid";
		else
			$sql = "select etp_form.formid, Count(etp_form_student.id) as dem from etp_form   JOIN etp_form_student  on etp_form.formid=etp_form_student.formid
where studentid='$mssv' and date1 >='$year-01-01' and date1<= '$year-07-30'
GROUP BY etp_form.formid";
